...
def crop_img(file,no_info_point=50):
    ...
    def remove_white_label(img):
        ...
        norm_border = np.append(np.reshape(img[:end_row+3,end_col+1:end_col+4],(-1,)),np.reshape(img[end_row+1:end_row+4,:end_col+3],(-1,)))
        img[:end_row,:end_col] = np.mean(norm_border)
        return img
    ...
    return img_grey_org[row_start:row_end,col_start:col_end]