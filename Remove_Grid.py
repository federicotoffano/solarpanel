def get_border_and_pad(dim,pixels_mov_avg = 20,increase_border = 6 ):
    ...
    print(1)
    print('mid border', first_mid_point)
    for i in range(1,dim-1):
        print(i)
        print('mid border', first_mid_point + i * (cell_thick+border_thick))
...
#%%
def remove_grid(medium_kernel = 15):
    # horizontal borders
    left_in,left_out,right_in,right_out,mid_points = get_border_and_pad(n_rows)
    ...
    plot(cropped_img,'border_rm_horizontal')
    ...
    plot(cropped_img,'border_rm_vertical')
    img_medianBlur = cv2.medianBlur(cropped_img,medium_kernel)
    plot(img_medianBlur,'img_medianBlur')
